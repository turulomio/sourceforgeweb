...
$cur=$mysqli->query("SELECT count(*), installations_uuid, date(datetime) as date, max(starts) FROM games, installations where games.installations_uuid=installations.uuid group by installations_uuid, datetime order by count(*) desc limit 10");
echo "<ul>";
while($row = $cur->fetch_array()) {
    echo "<li>" . $row[0] . " games played in installation since " . $row[2] . ": " . $row[1] . ". Last was at " . $row[3]. "</li>";
}
echo "</ul>";

echo "<h2>Top players in the last 30 days</h2>";
$cur=$mysqli->query("SELECT count(*), installations_uuid, date(datetime) as date, max(starts) FROM games, installations where games.installations_uuid=installations.uuid and starts>=DATE(NOW()) - INTERVAL 30 DAY group by installations_uuid, datetime order by count(*) desc limit 10 ");
echo "<ul>";
while($row = $cur->fetch_array()) {
    echo "<li>" . $row[0] . " games played in installation created at " . $row[2] . ": " . $row[1] . ". Last was at " . $row[3] . "</li>";
}
echo "</ul>";
